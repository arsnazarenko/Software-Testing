...
public class BTreeNode {
    ...
    private final int[] keys; // keys of nodes
    private final int MinDeg; // Minimum degree of B-tree node
    private final BTreeNode[] children; // Child node
    private int num; // Number of keys of node
    private final boolean isLeaf; // True when leaf node
    ...
    public List<Integer> traverse() {
        ...
        for (i = 0; i < num; i++) {
            if (!isLeaf) {
                list.addAll(children[i].traverse());
            }
            list.add(keys[i]);
